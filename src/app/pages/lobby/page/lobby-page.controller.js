...
function LobbyPageController($q, $mdDialog, $scope, $state, $window,
  ...
  vm.kick = function (playerSummary) {
    LobbyService.kick(vm.lobbyInformation.id, playerSummary.studentid);
  };

  vm.ban = function (playerSummary) {
    LobbyService.ban(vm.lobbyInformation.id, playerSummary.studentid);
  };
  ...
  vm.promote = function (player) {
    ...
    $mdDialog
      .show(confirmBox)
      .then(function () {
        LobbyService.setLobbyLeader(vm.lobbyInformation.id, player.studentid);
      });
  };
