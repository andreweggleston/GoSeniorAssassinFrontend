...
/** @ngInject */
function CommentBoxController($rootScope, $scope, $window, $log: AngularJSLog, $timeout: AngularJSTimeout,
                              ChatService) {
  var vm = this;
  ...
  var clearChatMessage = $rootScope.$on('chat-message', function (e, message) {
    function isFresh(msg) {
      return msg.id > latest && msg.player.studentid !== studentid;
    }
    ...
    if ($rootScope.userProfile) {
      var studentid = $rootScope.userProfile.studentid;
